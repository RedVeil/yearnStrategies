...
abstract contract StrategyConvexBase is BaseStrategy {
  ...
  // Swap stuff
  address internal constant sushiswap = 0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F; // default to sushiswap, more CRV and CVX liquidity there

  IERC20 internal constant crv = IERC20(0xD533a949740bb3306d119CC777fa900bA034cd52);
  IERC20 internal constant convexToken = IERC20(0x4e3FBD56CD56c3e72c1403e103b45Db9da5B9D2B);
  ...
  function setHarvestProfitNeeded(uint256 _harvestProfitMin, uint256 _harvestProfitMax) external onlyAuthorized {
...
// File: StrategyConvexEURSUSDC.sol
...
contract StrategyConvexEURSUSDC is StrategyConvexBase {
  ...
  // Curve stuff
  ICurveFi public constant curve = ICurveFi(0x98a7F18d4E56Cfe84E3D081B40001B3d5bD3eB8B); // This is our pool specific to this vault.
  ...
  constructor(
    ...
    (address lptoken, , , address _rewardsContract, , ) = IConvexDeposit(depositContract).poolInfo(_pid);
  ...
  function _sellCrvAndCvx(uint256 _crvAmount, uint256 _convexAmount) internal {
    ...
    if (_crvAmount > 0) {
      IUniV3(uniswapv3).exactInput(
        IUniV3.ExactInputParams(
          abi.encodePacked(address(crv), uint24(uniCrvFee), address(weth)),
          address(this),
          block.timestamp,
          _crvAmount,
          uint256(1)
    ...
    if (_wethBalance > 0) {
      if (optimal == 0) {
        IUniV3(uniswapv3).exactInput(
          IUniV3.ExactInputParams(
            abi.encodePacked(address(weth), uint24(uniUsdcFee), address(usdc)),
            address(this),
            block.timestamp,
            _wethBalance,
            uint256(1)
        ...
        IUniV3(uniswapv3).exactInput(
          IUniV3.ExactInputParams(
            abi.encodePacked(address(weth), uint24(uniUsdcFee), address(usdc), uint24(uniEursFee), address(eurs)),
            address(this),
            block.timestamp,
            _wethBalance,
            uint256(1)
  ...
  function claimableProfitInUsdt() internal view returns (uint256) {
    ...
    uint256 crvValue;
    if (_claimableBal > 0) {
      uint256[] memory crvSwap = IUniswapV2Router02(sushiswap).getAmountsOut(_claimableBal, usd_path);
    ...
    uint256 cvxValue;
    if (mintableCvx > 0) {
      uint256[] memory cvxSwap = IUniswapV2Router02(sushiswap).getAmountsOut(mintableCvx, usd_path);
